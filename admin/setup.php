...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $config = [
        'site_url' => sanitize_text($_POST['site_url'] ?? 'http://localhost:8090', 200),
        'admin_username' => $username !== '' ? $username : 'admin',
        'admin_password_hash' => '',
        'mail_to' => sanitize_text($_POST['mail_to'] ?? 'hello@example.com', 190),
        'mail_from' => sanitize_text($_POST['mail_from'] ?? 'noreply@example.com', 190),
...
<div class="admin-card admin-card--narrow">
  ...
  <form method="post" class="admin-form">
    ...
    <h2>Site</h2>
    <label>Site URL
      <input type="url" name="site_url" value="http://localhost:4321" required>
      <p class="admin-help">Local dev: use <code>http://localhost:4321</code> (Astro). The CMS admin runs on port 8090.</p>
    </label>

    <h2>Email (AWS SES SMTP)</h2>
    <label>Send enquiries to
      <input type="email" name="mail_to" value="hello@example.com" required>
    </label>
    <label>From email (must be verified in SES)
      <input type="email" name="mail_from" value="noreply@example.com" required>
    </label>
    <label>From name
